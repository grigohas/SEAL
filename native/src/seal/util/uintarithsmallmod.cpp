        ...
        void vector_mult_accumulate_u64_to_u128(const uint64_t* op1, const uint64_t* op2, size_t count, long long* acc_out) {
                   uint64_t acc_lo = 0;
                    ...
                    while (i < count) {
                        ...
                        // 64-bit multiplication (low and high parts)
                        vuint64m4_t vlo = __riscv_vmul_vv_u64m4(vop1, vop2, vl);     // lower 64 bits
                        vuint64m4_t vhi = __riscv_vmulhu_vv_u64m4(vop1, vop2, vl);   // upper 64 bits
                
                        // Reduce to scalars
                        vuint64m1_t vlo_sum_v = __riscv_vredsum_vs_u64m4_u64m1(vlo, __riscv_vmv_v_x_u64m1(0, 1), vl);
                        vuint64m1_t vhi_sum_v = __riscv_vredsum_vs_u64m4_u64m1(vhi, __riscv_vmv_v_x_u64m1(0, 1), vl);
                
                        uint64_t chunk_lo = __riscv_vmv_x_s_u64m1_u64(vlo_sum_v);
                        uint64_t chunk_hi = __riscv_vmv_x_s_u64m1_u64(vhi_sum_v);
                
                        // Add chunk_lo to acc_lo, check for carry
                        uint64_t prev_lo = acc_lo;
                        acc_lo += chunk_lo;
                        if (acc_lo < prev_lo) {
                            // Overflow occurred → carry
                            chunk_hi += 1;
                        }
                    ...
                    acc_out[0] = static_cast<long long>(acc_lo);  // low 64 bits
                    acc_out[1] = static_cast<long long>(acc_hi);  // high 64 bits (carry)
                }
        #endif
